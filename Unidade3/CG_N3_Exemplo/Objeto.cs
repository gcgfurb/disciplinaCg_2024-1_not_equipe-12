...
namespace gcgcg
{
  internal class Objeto  //TODO: deveria ser uma class abstract ..??
  {
    ...
    public void PontoAlterarMaisProximo(Ponto4D referencia, Ponto4D alvo = null){
      int indexParaAlterar = 0;
      double distanciaMinima = Matematica.DistanciaQuadrado(referencia, pontosLista[0]);
      for (int i = 1; i < pontosLista.Count; i++)
      {
        double distanciaAtual = Matematica.DistanciaQuadrado(pontosLista[i], referencia);
      ...
      this.PontosAlterar(alvo == null? referencia : alvo, indexParaAlterar);
    }

    public void PontoRemoverMaisProximo(Ponto4D referencia){
      ...
      pontosLista.RemoveAt(indexParaRemover);
      ObjetoAtualizar();
    }
    ...
    public bool Dentro(Ponto4D ponto){
      ...
      {
        Ponto4D proximoPonto = i == pontosLista.Count - 1? pontosLista[0] : pontosLista[i + 1];
        if(Matematica.ScanLine(ponto, pontosLista[i], proximoPonto))
          ScanLineCount++;
      }
      return ScanLineCount % 2 == 1;
    }
    #endregion
